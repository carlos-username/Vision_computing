...
mask_long=len(Sx) #getting mask length
print "ancho: ",ancho
print "alto: ",alto
angulos=[]
...
def main():
    ...
    r=0
    for i in xrange(1,alto-1): #Detecting edges by using previously calculated threshold
        for j in xrange(1,ancho-1):
            if histo[r][0]>threshold:
                ...
                img[i][j]=0
            r+=1
    plt.imshow(img,cmap = cm.Greys) #Drawing modified image with detected shapes
    plt.show()
